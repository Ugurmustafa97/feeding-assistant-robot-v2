...
% Required time for open mouth detection.
reqiredTimeMouth = 3;

% Counter for open mouth
counterMouth = 0;
        
%% SECTION FOR TRACKING THE MOUTH
while 1
    % Get the next frame.
    videoFrame = snapshot(cam);
    ...
    if  ~isempty(scoresMouth(idxMouth))
        % Write the confidence score of the left eye detection
        videoFrame = insertObjectAnnotation(videoFrame,'rectangle',bboxesMouth(idxMouth,:),cellstr(labelMouth(idxMouth)));
        ...
        if ~isempty(deltaVector)
            ...
            % first, the system aligns the arm in x-axis.
            if ((deltaVector(1) - biasInHorizontal) > thresholdInX) || ((deltaVector(1) - biasInHorizontal) < -thresholdInX)
                ...
                if ((deltaVector(1) - biasInHorizontal) > (thresholdInX))
...
%% SECTION FOR DETECTING OPEN MOUTH
